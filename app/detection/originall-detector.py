    ...
    def process_frame(self, frame, frame_size=None):
        try:
            ...
            if bboxs and len(bboxs[0]) > 0:
                ...
                if self.databases and detections:
                    utc_time = datetime.now(pytz.UTC)
                    local_time = utc_time.astimezone(self.local_tz)
                
                    for detection in detections:
                    # Prepare timestamps as strings for InfluxDB
                        detection_data = {
                            'text': detection['text'],
                            'confidence': detection['confidence'],
                            'timestamp_utc': utc_time.isoformat(),  # Convert to ISO format string
                            'timestamp_local': local_time.isoformat()  # Convert to ISO format string
                        }
                    ...
                    # Prepare timestamps as datetime objects for PostgreSQL
                        postgres_data = {
                            'text': detection['text'],
                            'confidence': detection['confidence'],
                            'timestamp_utc': utc_time,  # Keep as datetime object
                            'timestamp_local': local_time  # Keep as datetime object
                        }
