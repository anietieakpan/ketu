...
class VehicleClassifier:
    ...
    # def _detect_color_enhanced(self, image: np.ndarray) -> Tuple[str, float]:
    #     """Enhanced color detection using multiple color spaces and techniques"""
    #     try:
    #         # Convert to multiple color spaces for better detection
    #         hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
    #         lab = cv2.cvtColor(image, cv2.COLOR_BGR2LAB)
            
    #         # Define comprehensive color ranges
    #         color_ranges = {
    #             'white': {
    #                 'hsv': ([0, 0, 200], [180, 30, 255]),
    #                 'lab': ([200, 110, 110], [255, 140, 140])
    ...
    #             },
    #             'red': {
    #                 'hsv': ([0, 70, 50], [10, 255, 255]),
    #                 'hsv2': ([170, 70, 50], [180, 255, 255])
    #             },
    #             'blue': {
    #                 'hsv': ([100, 70, 50], [130, 255, 255])
    #             },
    #             'green': {
    ...
    #                 mask = cv2.inRange(hsv, np.array(ranges['hsv'][0]), 
    #                                  np.array(ranges['hsv'][1]))
    #                 if 'hsv2' in ranges:  # For wrapped hue (red)
    #                     mask2 = cv2.inRange(hsv, np.array(ranges['hsv2'][0]), 
    #                                       np.array(ranges['hsv2'][1]))
    ...
    #             color_scores[color_name] = score / len(ranges)
